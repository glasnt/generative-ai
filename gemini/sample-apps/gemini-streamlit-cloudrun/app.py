...
def get_model_name(model: GenerativeModel) -> str:
    ...
    model_name = model._model_name.replace(
...
with tab1:
    ...
    prompt = f"""\
        ...
        then make sure to have 5 chapters or else if it is "long" then 10
        chapters. Important point is that each chapters should be generated
        based on the premise given above. First start by giving the book
        introduction, chapter introductions and then each chapter. It should
        also have a proper ending. The book should have prologue and epilogue.
        """
...
with tab2:
    ...
    prompt = f"""\
        Generate a marketing campaign for {product_name}, a {product_category}
        designed for the age group: {target_audience_age}.
        ...
        Give proper bullet points and headlines for the marketing campaign. Do
        not produce any empty lines. Be very succinct and to the point.
        """
...
with tab3:
    ...
    with image_undst:
        st.markdown(dedent("""\
            In this demo, you will be presented with a scene (e.g., a living
            room) and will use the Gemini 1.5 Pro model to perform visual
            understanding. You will see how Gemini 1.0 can be used to recommend
            an item (e.g., a chair) from a list of furniture options as input.
            You can use Gemini 1.5 Pro to recommend a chair that would
            complement the given scene and will be provided with its rationale
            for such selections from the provided list.
            """)
        )
        image_bucket = "gs://github-repo/img/gemini/retail-recommendations"

        room_image_uri = f"{image_bucket}/rooms/living_room.jpeg"
        chair_1_image_uri = f"{image_bucket}/furnitures/chair1.jpeg"
        chair_2_image_uri = f"{image_bucket}/furnitures/chair2.jpeg"
        chair_3_image_uri = f"{image_bucket}/furnitures/chair3.jpeg"
        chair_4_image_uri = f"{image_bucket}/furnitures/chair4.jpeg"
        ...
        content = [
            ...
            room_image,
            (
            """Only recommend for the room provided and not other rooms. Provide
            your recommendation in a table format with chair name and reason as columns."""
            ),
        ]
    ...
    with screens_undst:
        ...
        st.write(
            """Equipped with the ability to extract information from visual elements on screens,
            Gemini 1.5 Pro can analyze screenshots, icons, and layouts to provide a holistic
            understanding of the depicted scene."""
        )
        ...
        prompt = """\
            How can I reset the clock on this appliance? Provide the
            instructions in English. If instructions include buttons, also
            explain where those buttons are physically located.
            """
        tab1, tab2 = st.tabs(["Response", "Prompt"])
    ...
    with diagrams_undst:
        ...
        st.write(
            """Gemini 1.5 Pro multimodal capabilities empower it to comprehend diagrams
            and take actionable steps, such as optimization or code generation. The
            following example demonstrates how Gemini 1.0 can decipher an Entity
    ...
    with recommendations:
        ...
        st.write(
            """Gemini 1.5 Pro is capable of image comparison and providing recommendations.
            This may be useful in industries like e-commerce and retail.
            Below is an example of choosing which pair of glasses would be better
            suited to various face types:"""
        )
    ...
    with sim_diff:
        ...
        st.write(
            """Gemini 1.5 Pro can also recognize math formulas and equations and extract
            specific information from them. This capability is particularly useful for
            generating explanations for math problems, as shown below."""
        )
...
with tab4:
    ...
    with video_highlights:
        st.markdown(
            """Below is another example of using Gemini 1.5 Pro to ask questions about
            objects, people or the context, as shown in the video about Pixel 8 below:"""
        )
